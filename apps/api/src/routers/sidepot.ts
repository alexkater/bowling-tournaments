...
export const sidepotRouter = router({
  ...
  join: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await Promise.all([
        ...
        assertTournamentPlayersInOrganization(ctx.db, [input.tournamentPlayerId], ctx.orgId),
      ]);

      return ctx.db.transaction(async (tx) => {
        const [lockedSidepot] = await tx
          .select({ id: sidepots.id })
          .from(sidepots)
          .where(eq(sidepots.id, input.sidepotId))
          .for('update')
          .limit(1);
        if (!lockedSidepot) {
          throw new TRPCError({ code: 'NOT_FOUND', message: 'Sidepot not found' });
        }

        return joinSidepotService(tx, input);
      });
    }),
  ...
  calculateResults: procedure
    ...
    .query(async ({ ctx, input: sidepotId }) => {
      await assertSidepotInOrganization(ctx.db, sidepotId, ctx.orgId);
      ...
      switch (sidepot.type) {
        case 'high_game': {
          ...
          const { prizePool, payout } = computePrizes(
            sidepot.entryFee,
            entries.length,
            config.payoutRatio,
          ...
          return {
            ...
            gameNumber,
            ...result,
            winners: result.winners.map((w) => ({ ...w, prize: payout })),
            prizePool,
          };
        }
        case 'high_series': {
          const result = calculateHighSeries(
            playerScores,
            config.gamesIncluded,
            config.handicap,
            handicapCfg,
          );
          const { prizePool, payout } = computePrizes(
          ...
          return {
            ...base,
            gamesIncluded: config.gamesIncluded,
            ...result,
            winners: result.winners.map((w) => ({ ...w, prize: payout })),
            prizePool,
          };
        ...
        case 'eliminator': {
          ...
          for (const gameNum of config.gamesIncluded) {
            ...
            const cut = calculateEliminatorCut(
              remaining,
              gameNum,
              advanceCount,
              config.handicap,
          ...
          return {
            ...base,
            rounds,
            winners: remaining.map((p) => ({ playerId: p.playerId, score: 0, prize: payout })),
            prizePool,
          };
        }
        case 'blind_draw': {
          ...
          const scoredPairs = pairs.map(([p1Id, p2Id]) => {
            ...
            const p1Hcp = config.handicap
              ? calculateHandicap(p1.average, DEFAULT_HANDICAP_CONFIG) * config.gamesIncluded.length
              : 0;
            const p2Hcp = config.handicap
              ? calculateHandicap(p2.average, DEFAULT_HANDICAP_CONFIG) * config.gamesIncluded.length
              : 0;
            const p1Total =
              config.gamesIncluded.reduce((s, g) => s + (p1.scores[g - 1] ?? 0), 0) + p1Hcp;
            const p2Total =
              config.gamesIncluded.reduce((s, g) => s + (p2.scores[g - 1] ?? 0), 0) + p2Hcp;
          ...
          return {
            ...base,
            pairs: scoredPairs,
            winners: topPairs.map((p) => ({ ...p, prize: payout })),
            prizePool,
          };
        }
        ...
        case 'sweeper_doubles': {
          const gameRounds = config.gamesIncluded.map((gameNum) => {
            ...
            const scoredPairs = pairs.map(([p1Id, p2Id]) => {
              ...
              if (!p1 || !p2) return { pair: [p1Id, p2Id] as [string, string], totalScore: 0 };
              const p1Hcp = config.handicap
                ? calculateHandicap(p1.average, DEFAULT_HANDICAP_CONFIG)
                : 0;
              const p2Hcp = config.handicap
                ? calculateHandicap(p2.average, DEFAULT_HANDICAP_CONFIG)
                : 0;
        ...
        default:
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: `Unknown sidepot type: ${sidepot.type}`,
          });
      }
    }),
    ...
    .query(async ({ ctx, input: sidepotId }) => {
      ...
      switch (sidepot.type) {
        ...
        case 'eliminator': {
          ...
          return {
            sidepotType: sidepot.type,
            winners: remaining.map((p) => ({ playerId: p.playerId })),
          };
        }
        case 'blind_draw': {
          const playerIds = playerScores.map((p) => p.playerId);
