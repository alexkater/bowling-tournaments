...
type DB = Pick<PostgresJsDatabase<typeof schema>, 'select' | 'insert'>;
...
export async function getSidepotOrThrow(db: DB, sidepotId: string) {
  const [sidepot] = await db.select().from(sidepots).where(eq(sidepots.id, sidepotId)).limit(1);
...
export async function getEntriesForSidepot(db: DB, sidepotId: string) {
  return db.select().from(sidepotEntries).where(eq(sidepotEntries.sidepotId, sidepotId));
}

export async function buildPlayerScores(db: DB, tpIds: string[]) {
  if (tpIds.length === 0) return [];

  const tps = await db.select().from(tournamentPlayers).where(inArray(tournamentPlayers.id, tpIds));
